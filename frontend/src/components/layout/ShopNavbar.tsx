...
import { Search, ShoppingCart, User, Moon, Sun, Menu, X, ChevronDown, Heart } from 'lucide-react';
      ...
      <header 
        className={`sticky top-0 w-full z-50 transition-all duration-500 ${
          isScrolled 
            ...
            : 'bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 py-6'
        }`}
      >
        <div className="container mx-auto px-4 lg:px-12 flex justify-between items-center">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <button 
              onClick={openCart} className="relative text-gray-600 hover:text-black dark:text-gray-300 dark:hover:text-white p-2 cursor-pointer transition-colors">
      ...
      <div className={`fixed inset-0 z-[100] transition-opacity duration-300 md:hidden ${isMobileOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`absolute top-0 right-0 h-full w-64 bg-white dark:bg-gray-900 shadow-2xl transform transition-transform duration-300 ease-in-out ${isMobileOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col p-6`}>
           ...
           <nav className="flex flex-col gap-6">
             ...
             <Link to="/products" onClick={() => setIsMobileOpen(false)} className={`text-lg font-medium ${location.pathname.startsWith('/products') ? 'font-bold text-black dark:text-white' : 'text-gray-600 dark:text-gray-300'}`}>Shop</Link>
