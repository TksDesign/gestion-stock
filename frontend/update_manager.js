...
const layoutContent = `import { Outlet, NavLink } from 'react-router-dom';
import { useAuthStore } from '../../store/authStore';
import { useThemeStore } from '../../store/themeStore';
import { LayoutDashboard, Package, CreditCard, Store, Moon, Sun, LogOut, Hexagon } from 'lucide-react';
...
export const ManagerLayout = () => {
  ...
  return (
    <div className="flex h-screen bg-[#F8F9FA] dark:bg-[#0A0A0A] overflow-hidden font-sans selection:bg-black selection:text-white dark:selection:bg-white dark:selection:text-black">
      {/* Sidebar */}
      <aside className="w-[280px] bg-white/80 dark:bg-black/50 backdrop-blur-2xl border-r border-gray-200/50 dark:border-white/5 flex flex-col transition-colors duration-300 relative z-10 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
...
fs.writeFileSync('src/components/layout/ManagerLayout.tsx', layoutContent);
console.log("Updated ManagerLayout.tsx");
